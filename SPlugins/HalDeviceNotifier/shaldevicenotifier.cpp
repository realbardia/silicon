...
void SHALDeviceNotifier::refresh()
{
    QString service = "org.freedesktop.Hal";
...
void SHALDeviceNotifier::detect( const QString & name )
{
    QString path = name;
    if( path.at(0) != '/' )
...
void SHALDeviceNotifier::detectDevice( const QString & name )
{
...
void SHALDeviceNotifier::detectDisc( const QString & name )
{
...
QVariant SHALDeviceNotifier::getProperty( const QString & device , const QString & key ) const
{
    if( device.isEmpty() )
        return QVariant();
...
bool SHALDeviceNotifier::mount( const SDeviceItem & device , const QString & mount_point )
{
    QString service = "org.freedesktop.Hal";
    QString path = device.currentDiscFeatures().info_udi_str;
    QString interface = "org.freedesktop.Hal.Device.Volume";
    QString method = "Mount";

    QDBusMessage Introspect_msg = QDBusMessage::createMethodCall( service , path , interface , method );
        Introspect_msg.setArguments( QList<QVariant>()<<QString()<<device.currentDiscFeatures().volume_fstype_str );
...
bool SHALDeviceNotifier::unmount( const SDeviceItem & device )
{
    QString service = "org.freedesktop.Hal";
    QString path = device.currentDiscFeatures().info_udi_str;
    QString interface = "org.freedesktop.Hal.Device.Storage";
    QString method = "Unmount";
...
void SHALDeviceNotifier::eject( const SDeviceItem & device )
{
    QString service = "org.freedesktop.Hal";
    QString path = device.deviceFeatures().storage_device_str;
    QString interface = "org.freedesktop.Hal.Device.Storage";
    QString method = "Eject";
...
const SDeviceFeatures & SHALDeviceNotifier::deviceFeatures( const SDeviceItem & device )
{
    QString name = p->device_list.key( device );
    return p->features_list[name];
}

const SDiscFeatures & SHALDeviceNotifier::discFeatures( const SDeviceItem & device )
{
    QString name = p->device_to_disc.value( p->device_list.key(device) );
    return p->disc_list[name];
}

QList<SDeviceItem> SHALDeviceNotifier::deviceList() const
{
...
SHALDeviceNotifier::~SHALDeviceNotifier()
{
